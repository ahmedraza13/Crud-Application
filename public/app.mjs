        ...
        .then(function (response) {
            ...
            response.data.map((eachPost) => {
                postsHtml +=
                    ...
                    <h3 class="user-info user-id">User ID:${eachPost.id}</h3>
                        <h3 class="post-title">Title:${eachPost.title}</h3>
                        <p class="post-text">${eachPost.text} </p>
                        <button class="edit-btn" onclick="delPost('${eachPost.id}')">Delete</button>
                        <button class="delete-btn" onclick="editPost('${eachPost.id}','${eachPost.title}','${eachPost.text}', )">Edit</button>
                    </div> 
                    <br />`
            })
...
window.editPost = (postId, title, text) => {
    ...
    document.querySelector(`#card-${postId}`).innerHTML =
        `<form onsubmit="savePost('${postId}')">
            ...
            <br/>
            <button class='post-tit'>Save</button>
